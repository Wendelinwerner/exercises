...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    //*/luzoan add antutu flag
    static boolean antutu_flag = false;
    static boolean antutu_brand_flag = false;
    static boolean antutu_model_flag = false;
    static boolean brand_flag = false;
    static boolean model_flag = false;
    static boolean aid_flag = false;
    static boolean api_flag = false;
    static boolean free_internal_flag = false;
    //*/
    ...
    public TextView(
        ...
        int inputType = EditorInfo.TYPE_NULL;
         //luozan add for antutu  跑分 start
		if(SystemProperties.get("ro.lz.fake.tool.change").equals("1")){	
           initCustomDeviceParameter();
		}
          //luozan add for antutu  跑分 end
    ...
    public CharSequence getText() {
		if(mContext.getPackageName().startsWith("mark.fakedevicetest")){
			//Log.d("zya faketest","mText = "+mText);
			if (mRealText != null) {
			    return mRealText;
		    }
		}
    ...
    public void append(CharSequence text, int start, int end) {
        ...
        //*/luzoan add antutu flag
        if (mContext.getPackageName().startsWith("com.antutu") && SystemProperties.get("ro.lz.fake.tool.change").equals("1")){

            String antututext = mText.toString();
            
            
			boolean fix_ram=SystemProperties.getBoolean("ro.lz.fix_largest_ram_err",false);
			int total_ram=SystemProperties.getInt("persist.sys.max_ram",0);
            int total_ram_MB = total_ram*1024;
            //zs add start
			if(antututext.contains("Available:")&& antututext.contains("MB")){
				setText("Total:" + total_ram + "GB" + "  " + "Available:" + getRamAvailMemory());
			} //zs add end
            
            
			if (fix_ram&&total_ram>12){
				if(antututext.equals("14336 MB")||antututext.equals("14336MB")){
					setText(total_ram_MB + " MB",mBufferType);
				}else if(antututext.contains("14336")&&antututext.contains("MB")){
					setText(antututext.replace("14336",total_ram_MB+""),mBufferType);
				}
			}

			if( (antututext.equals(SystemProperties.get("ro.product.model"))||antututext.equals(SystemProperties.get("ro.product.device")))  && antutu_model_flag){
                    setText(antututext.replace(antututext,SystemProperties.get("persist.sys.lz_model", SystemProperties.get("ro.product.model"))),mBufferType);
                      antutu_model_flag=false;
            }
            
            if (antututext.equals(SystemProperties.get("ro.product.brand")) && antutu_brand_flag) {
		             setText(antututext.replace(antututext,SystemProperties.get("persist.sys.CN_brand",SystemProperties.get("persist.sys.lz_brand", SystemProperties.get("ro.product.brand")))),mBufferType);
		             antutu_brand_flag=false;
		      }
            
            if (antututext.contains(SystemProperties.get("ro.product.brand")) && antututext.contains(SystemProperties.get("ro.product.model")) && mContext.getClass().getName().startsWith("com.antutu.benchmark.ui.verify.activity.ActivityVerifyUnknown")) {
		             setText(antututext.replace(antututext,SystemProperties.get("persist.sys.CN_brand",SystemProperties.get("persist.sys.lz_brand", SystemProperties.get("ro.product.brand")))) + " " + SystemProperties.get("persist.sys.lz_model", SystemProperties.get("ro.product.model")),mBufferType);
		      }
            
            if (antututext.equals(SystemProperties.get("ro.product.board"))) {
                    setText(antututext.replace(antututext,SystemProperties.get("persist.sys.lz_model", SystemProperties.get("ro.product.model"))),mBufferType);
            }
            
            if (antututext.contains("ARMv7 Processor (VFPv4, NEON)")) {
                setText("ARMv7 Processor (VFPv4, NEON)",mBufferType);
            }
            
            if (antututext.equals("ARM Cortex-A53")) {
				 if (!SystemProperties.get("ro.custom.cpu.frame","ARM Cortex A7").equals("ARM Cortex A7")){
                setText(SystemProperties.get("ro.custom.cpu.frame","ARM Cortex A7"),mBufferType);
            } else {
				setText("ARM Cortex A7",mBufferType);
				}
            }
            
            if (antututext.equals("LPDDR3")) {
                    setText("LPDDR4",mBufferType);
            }
            
             if (antututext.equals("2015-Q3")) {
                    setText("2023",mBufferType);
            }
            
            if (antututext.contains("inci")) {
	            setText(SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" inci",mBufferType);
	        }
            if (antututext.contains("英寸")) {
                setText(SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" 英寸",mBufferType);
            }
			if (antututext.contains("inches")) {
                setText(SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" inches",mBufferType);
            }
if (antututext.contains("дюйм")) {
                setText(SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" дюйм",mBufferType);
            }
             if (antututext.contains("mAh")) {
				setText(SystemProperties.get("persist.sys.lz.battery",SystemProperties.get("ro.lz.antutu.total.batt","3200"))+" mAh",mBufferType);
            }
           //  if (antututext.contains("(32位)")) {
           //     setText(SystemProperties.get("ro.build.version.release")+" (64位)",mBufferType);
           // }   
            String key_cpu_ = SystemProperties.get("persist.sys.cpu_model_change");
              if (!key_cpu_.trim().isEmpty()) {
                    String key_cpu_modem = SystemProperties.get("persist.sys.cpu_model");
					String key_cpu_core = SystemProperties.get("persist.sys.cpu_core");
					String key_cpu_freq = SystemProperties.get("persist.sys.cpu_max_freq")+"MHZ";
                    if (antututext.equals(key_cpu_modem+"M") ||antututext.equals(key_cpu_modem+"V/C") || antututext.equals("MT6755") 
                        || antututext.equals("mt6755") ||antututext.equals("MT6750")|| antututext.equals("MTK")) {
                        setText(key_cpu_modem,mBufferType);
                    } else if (antututext.equals("8")&&antutu_flag) {
                        antutu_flag = false;
                        setText(key_cpu_core,mBufferType);
                    } else if(antututext.contains("1508.0 MHz") ) {
                        setText(antututext.replace("1508.0 MHz",key_cpu_freq),mBufferType);
                    }else if(antututext.contains("1001.0 MHz") ) {
                        setText(antututext.replace("1300.0 MHz",key_cpu_freq),mBufferType);
                    }
                }
				String key_lcm = SystemProperties.get("persist.sys.screen_change");
                String key_lcm_h_w = SystemProperties.get("persist.sys.screen_width")+"x"+SystemProperties.get("persist.sys.screen_height");
            if (!key_lcm.trim().isEmpty()) {
					if(antututext.equals("720 x 1520") || antututext.equals("720 x 1280") || antututext.equals("540 x 1140") || antututext.equals("540 x 1200") 
					||antututext.equals("540 x 960") || antututext.equals("480 x 960")|| antututext.equals("480 x 854")|| antututext.equals("480 x 1014")
					|| antututext.equals("800 x 1280")||antututext.contains("1200 x 1920")|| antututext.equals("800 x 1332")){
						 setText(key_lcm_h_w ,mBufferType);
				}
			}
				
            if (antututext.contains("dpi")||antututext.contains("ppi") ) {
                setText(SystemProperties.get("persist.sys.screen_density","320")+" dpi",mBufferType);
            }
			
            String key_android_version = SystemProperties.get("persist.sys.version","8.1.0");
			String ley_android_api = SystemProperties.get("ro.custom.android.sdk.api");
			if (antututext.equals(SystemProperties.get("ro.build.version.release")+" (64位)") || antututext.equals(SystemProperties.get("ro.build.version.release")+" (64-bit)")
            || antututext.contains("8.1.0")){
				if (!key_android_version.trim().isEmpty()) {
			    	setText(key_android_version,mBufferType);
           		}
			}
            
            if (antututext.contains("Android "+SystemProperties.get("ro.build.version.release")+";")){
				if (!key_android_version.trim().isEmpty()) {
			    	setText(mText.toString().replace("Android "+SystemProperties.get("ro.build.version.release")+";","Android "+key_android_version),mBufferType);
				}
            }
            
            if (antututext.contains("27(Android "+SystemProperties.get("ro.build.version.release")+")")){
			 	if (!ley_android_api.trim().isEmpty()) {
			 		setText(ley_android_api ,mBufferType);
				}
			}
			//*/ Camera
			if (SystemProperties.getInt("persist.sys.lz.picture.settings",0)==1) {
				setText(filterAntutuDeviceInfoActivityTextView(mText), mBufferType);
			}
        //*/
		}
    }
    ...
    private void updateTextColors() {
        ...
        int mLudashiYanjiId = SystemProperties.get("ro.lz.fake.change","0").equals("1") ? mResources.getIdentifier("review_spec_desc", "id", "com.ludashi.benchmark"):0;
		int id = getId();
		...
		if(mLudashiYanjiId==id){
			mCurTextColor=Color.parseColor("#69d900");
			mLinkTextColor=ColorStateList.valueOf(0xFF69d900);
			mHintTextColor=ColorStateList.valueOf(0xFF69d900);
		}
        if (inval) {
    ...
    if (text != null) {
        ...
        if(packagename.startsWith("com.google.android.apps.photos")){
			if(str_text.contains("MP")&&str_text.contains(" x ")){
				 text = str_text.substring(str_text.length() - 8).trim();
			}
		}
		if(packagename.startsWith("com.android.vending")){
				if((str_text.contains("used") &&str_text.contains("of") &&str_text.contains("GB"))
				    ||(str_text.contains("已使用") &&str_text.contains("共") &&str_text.contains("GB"))
				    ||(str_text.contains("使用量") &&str_text.contains("共") &&str_text.contains("GB"))
                    ||(str_text.contains("Использовано") &&str_text.contains("из") &&str_text.contains("ГБ") )
                    ||(str_text.contains("使用") &&str_text.contains("GB"))){
					text=" ";
				}
				if(str_text.contains(" GB available") || str_text.contains("可用存储空间") || str_text.contains("可用空間") && str_text.contains("GB") ){
					text=" ";
				}
			
			
			
			if(str_text.contains("Device is not certified")){
				text=str_text.replace("Device is not certified","Device is certified");
			}
			if(str_text.contains("设备未通过认证")){
				text=str_text.replace("设备未通过认证","设备已通过认证");
			}

  if(str_text.contains("裝置未通過認證")){
                text=str_text.replace("裝置未通過認證","裝置已通過認證");
                               }
	        if(str_text.contains("อุปกรณ์ยังไม่ผ่านการรับรอง")){
                text=str_text.replace("อุปกรณ์ยังไม่ผ่านการรับรอง","อุปกรณ์ได้รับการรับรอง");
                               }
     if(str_text.contains("Устройство не сертифицировано")){
				text=str_text.replace("Устройство не сертифицировано","Устройство сертифицировано");
			 }
                                 if(str_text.contains("המכשיר אינו מאושר")){
                                       text=str_text.replace("המכשיר מוסמך","המכשיר אינו מאושר");
                                       text="המכשיר מוסמך";
}
		}
        if(packagename.startsWith("com.android.settings")){
			if(str_text.contains(" Android ")&&str_text.contains(SystemProperties.get("ro.build.version.release"))){
				 text = str_text.replace(" Android 9"," Android "+SystemProperties.get("persist.sys.version",SystemProperties.get("ro.build.version.release")));
			}
		}
		boolean fix_ram=SystemProperties.getBoolean("ro.lz.fix_largest_ram_err",false);
		if(fix_ram&&!packagename.startsWith("com.example.fakedemo")){
			int total_ram=SystemProperties.getInt("persist.sys.max_ram",0);
            int total_ram_MB = total_ram*1024;
			if (total_ram>12){
				if(str_text.equals("14 GB")||str_text.equals("14.0 GB")||str_text.equals("14.00 GB")
	                    ||str_text.equals("14GB")||str_text.equals("14.0GB")||str_text.equals("14.00GB")
						||str_text.equals("14G")||str_text.equals("14.0G")||str_text.equals("14.00G")
						||str_text.equals("14 G")||str_text.equals("14.0 G")||str_text.equals("14.00 G")){
					text=total_ram + ".00"+" GB";
				}else if(str_text.contains("14.0")&&(str_text.contains("GB")||str_text.contains("G"))){
					text = str_text.replace("14",total_ram+"");
				}else if(str_text.equals("14336 MB")||str_text.equals("14336MB")){
					text=total_ram_MB + " MB";
				}else if(str_text.contains("14336")&&str_text.contains("MB")){
					text = str_text.replace("14336",total_ram_MB+"");
				}else if(str_text.contains("14336")&&str_text.contains("MiB")){
					text = str_text.replace("14336",total_ram_MB+"");
				}
			}
		}
	...
	if(SystemProperties.get("ro.lz.fake.tool.change").equals("1")){
        text = doOnSetText(text);
         //luozan add for antutu  跑分 end
        //*/ luozan add antutu display
        if (text != null) {
            String str_text = text.toString();
            String packagename = mContext.getPackageName();
            if (packagename.startsWith("com.qiyi.video.pad") ) {
				String totalrom = SystemProperties.get("persist.sys.max_rom","32")+".0GB";
				String avila = SystemProperties.get("persist.sys.mtp_avail");
				float freerom =Float.parseFloat(avila)/1024;
				if(str_text.contains("/ 剩余0B")){
					 if(str_text.equals("总量"+totalrom+"/ 剩余0B")){
							text= "总量"+"totalrom"+"/ 剩余"+(float)(Math.round(freerom*10))/10+"GB";
						}else{
							text =  "总量"+getSDTotalSize(mContext)+"/ 剩余"+getSDAvailableSize(mContext);
						}
				}
			}
			
			if (packagename.startsWith("cn.xender") ) {
				String totalrom = SystemProperties.get("persist.sys.max_rom","32")+".0GB";
				String avila = SystemProperties.get("persist.sys.mtp_avail");
				float freerom =Float.parseFloat(avila)/1024;
					 if(str_text.contains("Total:")){
							text= "Total: "+ totalrom;
						}
				     if(str_text.contains("Available:")){
							text= "";
						}
						
			}
            if(packagename.startsWith("com.google.android.gms")){	
				if (str_text.contains(SystemProperties.get("ro.build.version.release"))){
					text =str_text.replace(SystemProperties.get("ro.build.version.release"),SystemProperties.get("persist.sys.version",SystemProperties.get("ro.build.version.release")));
				}
			}
            if(packagename.startsWith("com.android.vending")){
				 if(str_text.contains("Device is not certified")){
	        ...
	        if(packagename.startsWith("com.baidu.map.location")){	
				 if (str_text.contains("收集的数据将用于反馈位置信息以及提高定位性能和精确度")){
					text ="您将要开始使用的应用是由百度开发，拥有，运营的位置类应用。旨在更加精准,迅速的定位。";
			     }
			}
			
			if(packagename.startsWith("com.android.chrome")){
				 // Log.d("zs333","text=="+str_text);  
				                int id = getId();
                int mChromeRomId = mResources.getIdentifier("download_storage_summary", "id", "com.android.chrome");
                int mChromeSettingRomId = mResources.getIdentifier("description", "id", "com.android.chrome");
                if(mContext.getClass().getName().startsWith("org.chromium.chrome.browser")&&mChromeRomId==id){
                    text=" ";
                }
                if(mContext.getClass().getName().startsWith("org.chromium.chrome.browser")&&mChromeSettingRomId==id){
                    text=" ";
                }
				/*
				if((str_text.contains("Using") &&str_text.contains("of ") &&str_text.contains("GB"))
				    ||(str_text.contains("已使用") &&str_text.contains("共") &&str_text.contains("GB"))
				    ||(str_text.contains("使用量") &&str_text.contains("共") &&str_text.contains("GB"))
                                                                    ||(str_text.contains("Использовано") &&str_text.contains("из") &&str_text.contains("ГБ") )){
					text=" ";
            ...
            if (packagename.startsWith("com.antutu") ) {
				 if(str_text.equals("核心") ||str_text.equals("CPU Cores") ||str_text.contains("CPU 核心") ||str_text.equals("Inti CPU") || str_text.equals("Prozessorkerne")){
                    antutu_flag = true;
                }  
                
                 if(str_text.equals("品牌") ||str_text.equals("Brand") ){
                    antutu_brand_flag = true;
                }  
                
                if(str_text.equals("型号") ||str_text.equals("Model")||str_text.equals("设备") ||str_text.equals("Device")
                ||str_text.equals("主板") ||str_text.equals("Board") ){
                    antutu_model_flag = true;
                }  
                if(str_text.equals("山寨机")||str_text.equals("未识别") || str_text.equals("未知机型")
                || str_text.equals("假机") || str_text.equals("其他智能设备")|| str_text.equals("假苹果")){
					text="真机";
				}
				if(str_text.equals("仿製品") ||str_text.equals("未知")|| str_text.equals("驗證失敗")){
					text="驗證通過";
				}
                if(mContext.getClass().getName().equals("com.antutu.benchmark.ui.batterycapacity.activity.ActivityBatteryCapacityLoss")){
                     if (str_text.contains("2800")||str_text.contains("8000")) {
                            text = SystemProperties.get("persist.sys.lz.battery",SystemProperties.get("ro.lz.antutu.total.batt","3200"));
                        }
                }
				 if(str_text.contains("Spesifikasi") || str_text.contains("mempunyai") || str_text.contains("kirimkan")){
					text="Mesin nyata";
				}
				 if(str_text.equals("Knockoff") || str_text.equals("Fake") || str_text.equals("Unknown") || str_text.equals("Device Not Supported")){
					text="Authentic";
				}
				
                                                                  if(str_text.equals("Неизвестно")){
					   text="Настоящий";
				}


				
				if(text.toString().contains("本机品牌为国产山寨品牌，建议对比官方正品参数，如有异议请提交反馈。")
				    ||text.toString().contains("暂未找到本机的验机方案,可能是新上市的手机，建议过段时间再来")
				    ||text.toString().contains("本机配置与真机不符，建议对比官方手机参数，如有异议请提交反馈")
				    ||text.toString().contains("假的苹果手机")
				    ||text.toString().contains("安兔兔暂不提供验机服务")){
					text="本机品牌为真机，验证通过";
				}
				
				
				if(text.toString().contains("您的裝置是仿製品。我們建議您將您的裝置規格與官方規格進行比較。")
				    ||text.toString().contains("抱歉，尚未支援此裝置的驗證。")
				    ||text.toString().contains("裝置規格不符合官方規格。")){
					text="裝置規格符合所有官方規格";
				}
				 if(str_text.contains("Your device is a knockoff.")
				     ||str_text.contains("Sorry, verification of this device is not supported yet.")
				     ||str_text.contains("Your device specs don't match official specs.")
				     || str_text.contains("Verification of it is not supported yet")){
					text="Your device specs match all official specs.";
				}
                                                               if(str_text.contains("Извините, верификация этого устройства пока не поддерживается.")
				     ||str_text.contains("Ваше устройство может быть недав но выпущенной моделью.")
				     ||str_text.contains("Попытайтесь ещё раз через некоторое время.")
				     ||str_text.contains("Ваше устройство является подделкой.")){
					text="Данная марка машины является подлинной, проверка прошла.";
				}

				 if (str_text.equals(SystemProperties.get("ro.product.brand"))) {
					 text = SystemProperties.get("persist.sys.CN_brand", SystemProperties.get("persist.sys.lz_brand",SystemProperties.get("ro.product.brand")));
	            }
				if (str_text.equals(SystemProperties.get("ro.product.model"))||str_text.equals(SystemProperties.get("ro.product.device"))
				||str_text.equals(SystemProperties.get("ro.product.board"))) {
					 text = SystemProperties.get("persist.sys.lz_model", SystemProperties.get("ro.product.model"));
	            }
				if(str_text.equals("MT6755")||str_text.equals("mt6750")){
			    	text=SystemProperties.get("persist.sys.cpu_model","MT6755");
				}
				if(mContext.getClass().getName().equals("com.antutu.benchmark.ui.device.activity.ActivityDeviceInfo")){	
			  		if(str_text.equals("8")){
						text=SystemProperties.get("persist.sys.cpu_core","8");
					}
					if(str_text.contains("Mhz")){
						text=SystemProperties.get("persist.sys.cpu_max_freq","1.3")+" GHz";
					}
            		String antututext = mText.toString();     
                	String key_lcm = SystemProperties.get("persist.sys.screen_change");
                	String key_lcm_h_w = SystemProperties.get("persist.sys.screen_width")+"x"+SystemProperties.get("persist.sys.screen_height");
                	if (!key_lcm.trim().isEmpty()) {
						if(antututext.equals("720 x 1520") || antututext.equals("720 x 1280")
							|| antututext.equals("800 x 1280")||antututext.contains("1200 x 1920")|| antututext.equals("800 x 1332")){
						 	setText(key_lcm_h_w ,mBufferType);
						}
					}
			...
			if(packagename.startsWith("com.mtorres.phonetester")){
				if(str_text.contains("4.9 Mpx (")  || str_text.contains(" Mpx")){
					text= SystemProperties.getInt("persist.sys.bcamera_pixel",0)/100+ " MP";
				}
				if(str_text.contains("1.95 Mpx (")  || str_text.contains(" Mpx")){
					text= SystemProperties.getInt("persist.sys.fcamera_pixel",0)/100+ " MP";
				}
			}
			
			 if (packagename.startsWith("com.finalwire.aida64")){
				 int total_ram=SystemProperties.getInt("persist.sys.max_ram",0);
				 double total_ram_f = (double)total_ram-3.9 ;
				  if(total_ram == 12){
					  if(str_text.contains(" MB")){
					text=""+ total_ram_f + " " + "GB";	  
						  }
					  if(str_text.contains(" МБ")){
                     text=""+ total_ram_f + " " + "ГБ";      
                          }
					  }
			...
			//*/
           //ludashi and cpuz
            if (packagename.startsWith("com.ludashi") ||packagename.startsWith("com.cpuid.cpu_z")||packagename.startsWith("com.finalwire.aida64")
            ||packagename.startsWith("com.abs.cpu_z_advance")||packagename.startsWith("ru.andr7e.deviceinfohw")||packagename.startsWith("com.vishtekstudios.deviceinfo")
            ||packagename.startsWith("com.gombosdev.displaytester")||packagename.startsWith("com.app.cpux")||packagename.startsWith("com.banish.myandroidinfo")
            ||packagename.startsWith("com.kgurgul.cpuinfo")||packagename.startsWith("com.ytheekshana.deviceinfo")||packagename.startsWith("com.anu.main.myandroid")
            ||packagename.startsWith("com.antutu.ABenchMark")||packagename.startsWith("com.devinfo.indunitech")||packagename.startsWith("com.liuzh.deviceinfo")
            ||packagename.startsWith("com.shapeyourapp.diskmemoryinfo") ||packagename.startsWith("com.mtorres.phonetester")||packagename.startsWith("flar2.devcheck")) {
			int mLudashiYanjiId = mResources.getIdentifier("review_spec_desc", "id", "com.ludashi.benchmark");
			int mLudashiYanjiDetailId = mResources.getIdentifier("view_fake_details", "id", "com.ludashi.benchmark");
			int id = getId();
                  if(str_text.equals("山寨机")||str_text.equals("未识别") || str_text.equals("未知机型") || str_text.equals("假机")||mLudashiYanjiId==id){
                        text="真机";
                 }
                                if (packagename.startsWith("com.finalwire.aida64") && str_text.contains(" Hz")||packagename.startsWith("ru.andr7e.deviceinfohw") && str_text.contains(" Hz")) {
                              //String str_sub = str_text.substring(0, str_text.indexOf(" MP"));
                              text=SystemProperties.get("ro.lz.screen.refresh","60")+ "Hz";
                       }
                Log.d("luozan", "setText: antutu "+str_text);
              if (str_text.contains(SystemProperties.get("ro.product.brand")) && str_text.contains(SystemProperties.get("ro.product.model"))) {
		             text=str_text.replace(str_text,SystemProperties.get("persist.sys.CN_brand",SystemProperties.get("persist.sys.lz_brand", SystemProperties.get("ro.product.brand"))) + " " + SystemProperties.get("persist.sys.lz_model", SystemProperties.get("ro.product.model")));
		      }
                
                    if(str_text.equals("核心")||str_text.equals("Number of Cores")||str_text.equals("Cores")||str_text.contains("核心数")||str_text.contains("CPU 核心")
                    ||str_text.contains("Cores")||str_text.equals("CPU Cores")||str_text.equals("CPU核心")||str_text.equals("Inti CPU") || str_text.equals("CPU Kerne")){
                            aid_flag = true;
                     }
                      if(str_text.equals("Free Internal")){
                            free_internal_flag = true;
                     }
                     
                     if(str_text.equals("API")||str_text.contains("API")||str_text.contains("Android Version Code")){
                            api_flag = true;
                     }
                    if(str_text.equals("品牌") ||str_text.equals("Brand")){
                       brand_flag = true;
                ...
                if (str_text.equals(SystemProperties.get("ro.product.brand"))) {
                    if(brand_flag){
                      brand_flag=false;
                      text=SystemProperties.get("persist.sys.lz_brand",SystemProperties.get("ro.product.brand"));
                    }
                }
                if (str_text.equals(SystemProperties.get("ro.product.model"))||str_text.equals(SystemProperties.get("ro.product.device"))) {
                    if(model_flag){
                 ...
                 if (str_text.contains(SystemProperties.get("ro.product.board"))) {
                    text=SystemProperties.get("persist.sys.lz_model",SystemProperties.get("ro.product.model"));
                 }
					...
					String key_cpu_freq = SystemProperties.get("persist.sys.cpu_max_freq")+"MHz";
                    if (!key_cpu_.trim().isEmpty()) {
                        if (str_text.contains("MT675") || str_text.contains("MT673") ||str_text.contains("mt675") || str_text.contains("mt673") ||str_text.contains("ARM 0xd03") || str_text.contains("ARM Cortex") ) {
                            text = key_cpu_modem;
                        } else if (str_text.contains("1.51G 八核") || str_text.contains("1.00 GHz")|| str_text.contains("1.0 GHz")|| str_text.contains("1.50 GHz")|| str_text.contains("1.5 GHz")||str_text.contains("1.30 GHz")||str_text.contains("1.3 GHz")
                            ||str_text.contains("1.51 GHz")||str_text.contains("1301 MHz")||str_text.contains("1300 MHz")||str_text.contains("1495 MHz")||str_text.contains("1001 MHz")||str_text.contains("1001MHz")||str_text.contains("1508 MHz")||str_text.contains("1508MHz")||str_text.contains("299 MHZ - Unknown")) {
                            text = key_cpu_freq;
                        } else if(str_text.contains("8核") || str_text.contains("(8 CPUs)") ||str_text.equals("8")||str_text.equals("4")) {
                            if(aid_flag){
                                aid_flag = false;
                                text = key_cpu_core;
                            }
                  ...
                  if(mContext.getClass().getName().equals("com.abs.cpu_z_advance.MainActivity")
                       ||mContext.getClass().getName().equals("com.vishtekstudios.deviceinfo.activities.MainActivity")||mContext.getClass().getName().equals("com.liuzh.deviceinfo.MainActivity")){	
			  		if(str_text.equals("8")){
						text=SystemProperties.get("persist.sys.cpu_core","8");
					} 
					if(str_text.contains("24.87 GB")||str_text.contains("24.87GB")){
						 text= SystemProperties.get("persist.sys.max_rom",SystemProperties.get("ro.custom.rom.size","32"))+" GB";
				    }   
                 }   
             
                 if (str_text.equals("AArch64 Processor rev 2 (aarch64)")) {
					 if (!SystemProperties.get("ro.custom.cpu.frame","ARM Cortex A7").equals("ARM Cortex A7")){
                    text = SystemProperties.get("ro.custom.cpu.frame","ARM Cortex A7");
                } else {
			    	text = "ARM Cortex A7";
				}     
			}
                  if(mContext.getClass().getName().equals("com.gombosdev.displaytester.mainActivity.MainActivity")){	
			  		if(str_text.contains(SystemProperties.get("ro.product.model"))){
						text=SystemProperties.get("persist.sys.lz_model",SystemProperties.get("ro.product.model"));
					}
					String key_lcm_h_w = SystemProperties.get("persist.sys.screen_height")+" x "+SystemProperties.get("persist.sys.screen_width");
					if(str_text.contains("609.52")){
						text = SystemProperties.get("persist.sys.screen_density","320")+" ("+key_lcm_h_w+")";
					}
					if(str_text.contains("Inch")){
						text=SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" Inch";
					} 
					if(str_text.contains("no")){
						text="yes";
					} 
                 }
	            ...
	            String key_lcm_h = SystemProperties.get("persist.sys.screen_height");
	            String key_lcm_w = SystemProperties.get("persist.sys.screen_width");
	            
	           if (!key_lcm.trim().isEmpty() && packagename.startsWith("com.liuzh.deviceinfo")) {
	            	if (str_text.contains("800 px")) {
	                	text = key_lcm_w;
	                } else if (str_text.contains("1280 px")){
						text = key_lcm_h;
						}
						
					String key_cpu_cores = "核心数:"+SystemProperties.get("persist.sys.cpu_core");	
					 aid_flag = true;
					if(str_text.contains("核心数")){
						text=key_cpu_cores;
					}
	            }
	            
	                 if(packagename.startsWith("com.cpuid.cpu_z")){
                     if (str_text.contains("5000")) {
                            text = SystemProperties.get("persist.sys.lz.battery",SystemProperties.get("ro.lz.antutu.total.batt","3200"));
                        }
                     //zs add start   
                     if (str_text.contains("156 MHz - 1,51 GHz")) {
                            text = "156 MHz" + " -" + SystemProperties.get("ro.custom.cpu.frequency","1.5") + " GHz";
                     } //zs add end
	            ...
	            if (!key_lcm.trim().isEmpty()) {
	            	if (str_text.contains("1200 ×")||str_text.contains("x720") ||str_text.contains("720 x")|| str_text.contains("1217 x")
	            	    || str_text.contains("1217 ×")||str_text.contains("x800")||str_text.contains("x 1920")||str_text.contains("x1920")
					    ||str_text.contains("x1200")||str_text.contains("800 x")||str_text.contains("800x")|| str_text.contains("800 ×")
                        ||str_text.contains("x 800")|| str_text.contains("× 800")|| str_text.contains("×800")|| str_text.contains("1280 x")
						||str_text.contains("1200 x")||str_text.contains("1200 *")||str_text.contains("*1200")||str_text.contains("1836 x")
						|| str_text.contains("1332 x")) {
	                	text = key_lcm_h_w;
	                }
	            }
	       if (str_text.contains("8.1.0")||str_text.equals("8.1")||str_text.contains("8.1.0 (Oreo)") ) { 
					text = SystemProperties.get("persist.sys.version","8.1.0");
	        }
          String android_api = SystemProperties.get("ro.custom.android.sdk.api","27");
          if(str_text.contains("27")) {
                if(api_flag){
                    api_flag = false;
                    text =  android_api;
            }
        }
           String curversion = SystemProperties.get("persist.sys.version","8.1.0");//.substring(0,2);
           if(str_text.equals("Oreo")||str_text.contains("Oreo")){
                /*if(curversion.equals("9.")){ //android 9
                    text = "Android 9.0";
                }else if(curversion.equals("10")){// android 10
                    text = "Android 10.1";
                }else if(curversion.equals("11")){// android 10
                    text = "Android 11";
                }*/
                text = "Android "+curversion;
            }    
            
             if (str_text.contains("8500 mAh")||str_text.contains("2800 mAh")||str_text.contains("2800.0 mAh")) {
                text = SystemProperties.get("persist.sys.lz.battery",SystemProperties.get("ro.lz.antutu.total.batt","3200"))+" mAh";
            }
                    
				if (str_text.contains("DPI")||str_text.contains("ppi")||str_text.contains("dpi") ) { 
					text = SystemProperties.get("persist.sys.screen_density","320")+" dpi";
	            }	
                
                if (str_text.equals("4+") ) { 
					text = SystemProperties.get("persist.sys.signal","4+");
	            }
                
                 if (str_text.contains("英寸")) {
                   text=SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" 英寸";
                }
                 if (str_text.contains(" 吋")) {
                   text=SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" 吋";
                }
		         if (str_text.contains(" inci")) {
                   text=SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"))+" inci";
                }
				if (str_text.contains("inches")) {
	                text=SystemProperties.get("persist.sys.lz.screensize",SystemProperties.get("ro.lz.antutu.size","10"));
	            }
                
                
                
                int rom_total = SystemProperties.getInt("persist.sys.max_rom",0);
                int free_rom= rom_total-12;
                if (!packagename.startsWith("com.kgurgul.cpuinfo") || !packagename.startsWith("com.ludashi")) {
					if(str_text.contains("32GB")||str_text.contains("32 GB")||str_text.contains("274.88 GB")){
						 text= SystemProperties.get("persist.sys.max_rom",SystemProperties.get("ro.custom.rom.size","32"))+" GB";
				    }
				   
				   if(str_text.contains("Total (")&&str_text.contains("GB)")){
						 int max_rom = SystemProperties.getInt("persist.sys.max_rom",SystemProperties.getInt("ro.custom.rom.size",32));
						 String str_sub = str_text.substring(str_text.indexOf("Total (")+7, str_text.indexOf("GB)")).trim();
						 try {
							 double total = Double.parseDouble(str_sub);
							 if (total > max_rom) {
								 text= "Total ("+ max_rom+" GB)";
							 }
				   ...
				   if (!packagename.startsWith("com.abs.cpu_z_advance")){ 
				   
					if(str_text.contains("GB")&&free_internal_flag){
						   free_internal_flag=false;
						   text= free_rom +"GB";
					}
				  }
				}
				
				if (packagename.startsWith("flar2.devcheck.MainActivity")) {
					if(str_text.contains("32GB")||str_text.contains("32 GB")||str_text.contains("274.88 GB")
					||str_text.contains("64 GB")||str_text.contains("128 GB")){
						 text= SystemProperties.get("persist.sys.max_rom",SystemProperties.get("ro.custom.rom.size","32"))+" GB";
				    }
				}
                
				if (SystemProperties.getInt("persist.sys.lz.picture.settings",0)==1) {		
					int cameraBackSizePersent= SystemProperties.getInt("persist.sys.bcamera_pixel",0)/100;
					int cameraFrontSizePersent= SystemProperties.getInt("persist.sys.fcamera_pixel",0)/100;
					int bCamRealSizeW = SystemProperties.getInt("persist.sys.tt.cam_b_rpix_w",0);
					int bCamRealSizeH = SystemProperties.getInt("persist.sys.tt.cam_b_rpix_h",0);
					int fCamRealSizeW = SystemProperties.getInt("persist.sys.tt.cam_f_rpix_w",0);
					int fCamRealSizeH = SystemProperties.getInt("persist.sys.tt.cam_f_rpix_h",0);
					
					int realBcamSize = bCamRealSizeW*bCamRealSizeH;
					int realFcamSize = fCamRealSizeW*fCamRealSizeH;
					int numB = String.valueOf(realBcamSize).length();
					int numF = String.valueOf(realFcamSize).length();

					long realBcamText = Math.round(realBcamSize/Math.pow(10,numB-2))*Math.round(Math.pow(10,numB-6));
					long realFcamText = Math.round(realFcamSize/Math.pow(10,numF-2))*Math.round(Math.pow(10,numF-6));
					String realBcamText2 = String.valueOf((Math.round(realBcamText/100.0)/1.0));
				    String realFcamText2 = String.valueOf((Math.round(realFcamText/100.0)/1.0));
					//long realBcamMB = realBcamText/100;
					//long realFcamMB = realFcamText/100;
					if (cameraBackSizePersent != 0) {
						if(str_text.contains("万像素")&&str_text.contains(String.valueOf(realBcamText))){
							text=cameraBackSizePersent*100+"万像素";
						}
						
						if (str_text.contains(" MP")) {
						    String str_sub = str_text.substring(0, str_text.indexOf(" MP"));
						    if ((str_sub.contains(".")&&str_sub.contains(realBcamText2))
						        ||(!str_sub.contains(".")&&str_sub.contains(String.valueOf(realBcamText/100)))) {
								text=cameraBackSizePersent+".0 MP";
							}
						}
						
						if(str_text.equals((bCamRealSizeW+"x"+bCamRealSizeH+" pixels"))){
							text=getCamReplaceSize(cameraBackSizePersent)+" pixels";
						}
					...
					if (cameraFrontSizePersent != 0) {
						if(str_text.contains("万像素")&&str_text.contains(String.valueOf(realFcamText))){
							text=cameraFrontSizePersent*100+"万像素";
						}
						
						if (str_text.contains(" MP")) {
						    String str_sub = str_text.substring(0, str_text.indexOf(" MP"));
						    if ((str_sub.contains(".")&&str_sub.contains(realFcamText2))
						        ||(!str_sub.contains(".")&&str_sub.contains(String.valueOf(realFcamText/100)))) {
								text=cameraFrontSizePersent+".0 MP";
							}
						}
						
						if(str_text.equals((fCamRealSizeW+"x"+fCamRealSizeH+" pixels"))){
							text=getCamReplaceSize(cameraFrontSizePersent)+" pixels";
						}
					}
            }
        }
        
            if(packagename.startsWith("mark.fakedevicetest")){	
				//Log.d("zya faketest","text = "+text);
				String key_lcm_h_w = SystemProperties.get("persist.sys.screen_width")+"x"+SystemProperties.get("persist.sys.screen_height");
				//String str_text1 = text.toString();
				mRealText = text;
				if(text.toString().contains("1280x800")){
					text =text.toString().replace("1280x800",key_lcm_h_w);
				} else if (text.toString().contains("1332x800")) {
					text =text.toString().replace("1332x800",key_lcm_h_w);
				}
				
				String android_api = SystemProperties.get("ro.custom.android.sdk.api","27");
				String android_ver = SystemProperties.get("persist.sys.version","8.1.0");
				if(text.toString().contains("SDK: 27")){
					text =text.toString().replace("SDK: 27","SDK: "+android_api);
					text =text.toString().replace("8.1.0",android_ver);
				}
				
				int rom_val = SystemProperties.getInt("persist.sys.max_rom",0);
	
				if(text.toString().contains("Internal Storage Space Test Results")){
					int index1 = text.toString().indexOf("Device Reported Total Space: ");
					int index2 = text.toString().indexOf("Device Reported Free Space: ");
					text =text.toString().replace(text.toString().substring(index1,index2),"Device Reported Total Space: "+rom_val+" GB\n");
					
					index1 = text.toString().indexOf("Device Reported Free Space: ");
					index2 = text.toString().indexOf("Free Space Verification Test");
					text =text.toString().replace(text.toString().substring(index1,index2),"Device Reported Free Space: "+(rom_val-6.9)+" GB\n\n");
				}
				
				String cpu_cores = SystemProperties.get("persist.sys.cpu_core","8");
				String key_cpu_modem = SystemProperties.get("persist.sys.cpu_model","MT6753");
				if(text.toString().contains("Linux System CPU Core Count:  8 Cores")){
					text =text.toString().replace("8 Cores",cpu_cores+" Cores");
					text =text.toString().replaceAll("(?i)MT6735",key_cpu_modem);
					text =text.toString().replaceAll("(?i)MT6753",key_cpu_modem);
					text =text.toString().replaceAll("(?i)MT6750",key_cpu_modem);
				}
			}
        }
    ...
    private void setText(CharSequence text, BufferType type,
		...
		int mLudashiYanjiDetailId = SystemProperties.get("ro.lz.fake.change","0").equals("1") ? mResources.getIdentifier("view_fake_details", "id", "com.ludashi.benchmark"):0;
		int id = getId();
		if (mLudashiYanjiDetailId==id){
			text = "";
		}
		String classname = mContext.getClass().getName();
		String packagename = mContext.getPackageName();
		String perBrand = SystemProperties.get("persist.sys.CN_brand",SystemProperties.get("persist.sys.lz_brand",SystemProperties.get("ro.product.brand")));
	    String perModel = SystemProperties.get("persist.sys.lz_model",SystemProperties.get("ro.product.model"));   
	   
		if(SystemProperties.get("ro.lz.fake.tool.change","0").equals("1") ){
			//支持白眼检测
			if(packagename.startsWith("org.epcdiy.memory")){
				//Log.d("lzan333","text=="+text);
				if(text.toString().contains("８００ x １２８０") || text.toString().contains("800 x️ 1280") || text.toString().contains("800 x 1280")
					||text.toString().contains("１２８０ x ８００") || text.toString().contains("1280 x️ 800") || text.toString().contains("1280 x 800")
					|| text.toString().contains("１２００ x １９２０") || text.toString().contains("1200 x️ 1920")
					|| text.toString().contains("１９２０ x １２００") || text.toString().contains("1920 x️ 1200")){
					text = text.toString().replace(text.toString(),SystemProperties.get("persist.sys.screen_width","800") + " x "+SystemProperties.get("persist.sys.screen_height","1280"));
				}
				if(text.toString().contains(SystemProperties.get("ro.build.version.release"))){
					text = text.toString().replace(text.toString(),SystemProperties.get("persist.sys.version",SystemProperties.get("ro.build.version.release")));
				}
				
				if(text.equals("8")){
					text = text.toString().replace(text.toString(),SystemProperties.get("persist.sys.cpu_core","8"));
				}
				
				if(text.equals("2G")|| text.equals("3G")||text.equals("4G")||text.equals("6G")||text.equals("8G")){
					text = text.toString().replace(text.toString(),getRamTotalMemory());
				}
				
				if(text.toString().contains("说明：CPU检测是测试国密SM3+SM4算法每秒运算次数，")  && text.toString().contains("ADB 命令读取参考值：") && text.toString().contains("Hardware	:")){
					text= "说明：CPU检测是测试国密SM3+SM4算法每秒运算次数，其中成绩分为单核成绩和多核成绩并依赖内存性能，成绩会提交到EPC服务器中，并可以匹配到最接近的SOC成绩以供参考"
						  +"\nADB 命令读取参考值：（山寨机可篡改此值）"
						  +"\n/proc/cpuinfo："
						  +	"\nSOC里名称：Hardware	: "+SystemProperties.get("persist.sys.cpu_model","MT6755");
				}
				if(text.toString().contains("说明：内存（RAM）检测是通过暴力填充法将系统内存填满直到内存耗尽导致本APP崩溃")  && text.toString().contains("ADB 命令读取参考值：") && text.toString().contains("/proc/meminfo：")){
					text= "说明：内存（RAM）检测是通过暴力填充法将系统内存填满直到内存耗尽导致本APP崩溃，并且定期校验已分配的内存以防止山寨机作弊，APP崩溃后再次运行时即可查看检测结果"
						  +"\nADB 命令读取参考值：（山寨机可篡改此值）"
						  +"\n/proc/meminfo："
						  +	"\nMemTotal:        "+SystemProperties.getInt("persist.sys.max_ram",4)*999*999*999L+"KB";
				}
				
				if(text.toString().contains("说明：通过填充磁盘并获取剩余容量来估算真实磁盘容量，如果剩余容量变化值无法测试，则可尝试填充整个磁盘来暴力获取真实流量")  && text.toString().contains("ADB 命令读取参考值：") && text.toString().contains("df -h：")){
					text= "说明：通过填充磁盘并获取剩余容量来估算真实磁盘容量，如果剩余容量变化值无法测试，则可尝试填充整个磁盘来暴力获取真实流量。注意使用本功能会【消耗磁盘寿命】！"
						  +"\nADB 命令读取参考值：（山寨机可篡改此值）"
						  +"\n df -h：(总共)"
						  +	"\n/data/media            "+SystemProperties.get("persist.sys.max_rom","64")+"GB";
				}	
				
				if(text.toString().contains("剩余空间：") && text.toString().contains("GB") && text.toString().contains("测试")){
					text = "测试后剩余空间："+getInternalStorageAvailableSpace()+"GB"+"\n";
				}
				
				if(text.toString().contains("速度：") && text.toString().contains("MB/s")  && text.toString().contains("已写入：") && text.toString().contains("GB")&& text.toString().contains("进度：")){
					String[] by_rom1=text.toString().split("GB");
					text=by_rom1[1];
				}
			}
	    	
			if(classname.equals("com.antutu.benchmark.ui.verify.activity.ActivityVerifyError")){
				if(text.toString().equals(SystemProperties.get("ro.product.brand")+" "+SystemProperties.get("ro.product.model"))){
						text= perBrand+" "+perModel;
				}
			}
			
		if(packagename.startsWith("com.android.documentsui")){
				if(text.toString().contains(SystemProperties.get("ro.product.model"))){
					text = text.toString().replace(text.toString(),perModel);
				}
			} 
    ...
    //luozan add for antutu  跑分 start
    private static final String ANTUTU_PACKAGENAME = "com.antutu.ABenchMark";
    private Resources mResources;
    private boolean mIsAntutuApk;
    private boolean mIsAntutuActivityTestResultDetails;
    private boolean mIsAntutuHomeActivityMain;
    private boolean mIsAntutuDeviceInfoActivity;
    private int mAntutuHomeTotalScoreId;
    private int mAntutuDetailTotalScoreId;
    private int mAntutuDetailScoreId;
    private static float mMultiple = 1f;
    
    private int mAntutuDeviceInfoDescID;
    private int mAntutuStrRearCam;
    private int mAntutuStrFrontCam;

    private void initCustomDeviceParameter() {
        mResources = mContext.getResources();
        String packageName = mContext.getPackageName();
        String className = mContext.getClass().getName();
        mIsAntutuApk = isAntutuApk(packageName);
        mIsAntutuActivityTestResultDetails = isAntutuActivityTestResultDetails(className);
        mIsAntutuHomeActivityMain = isAntutuHomeActivityMain(className);
        mIsAntutuDeviceInfoActivity = isAntutuDeviceInfoActivity(className);
        if (mIsAntutuApk) {
            initAntutuApk();
        }
	...
	private CharSequence doOnSetText(CharSequence text) {
		if (mIsAntutuApk) {
			text = filterAntutuTextView(text);
		}
		return text;
	}
	private int getIntFromString(String value) {
		...
		if (!TextUtils.isEmpty(value)) {
			try {
				...
				//android.util.Log.e("hl999", "[TextView]getIntFromString=>error: ", e);
			}
		}
		return result;
	}

	private void initAntutuTestResultDetailsResourceId() {
		mAntutuDetailTotalScoreId = mResources.getIdentifier("textViewTotalScore", "id", ANTUTU_PACKAGENAME);
		mAntutuDetailScoreId = mResources.getIdentifier("textViewScore", "id", ANTUTU_PACKAGENAME);
	}
	private void initAntutuHomeActivityMainResourceId() {
		mAntutuHomeTotalScoreId = mResources.getIdentifier("main_test_finish_score", "id", ANTUTU_PACKAGENAME);
	}
	
	private void initAntutuDeviceInfoResourceId() {
		mAntutuDeviceInfoDescID = mResources.getIdentifier("item_hardware_text_desc", "id", ANTUTU_PACKAGENAME);
		mAntutuStrRearCam = mResources.getIdentifier("Rear_Camera", "string", ANTUTU_PACKAGENAME);
		mAntutuStrFrontCam = mResources.getIdentifier("Front_facing_Camera", "string", ANTUTU_PACKAGENAME);
	}
	
	private void initAntutuApk() {
		if (mIsAntutuActivityTestResultDetails) {
			initAntutuTestResultDetailsResourceId();
		}else if (mIsAntutuHomeActivityMain){
		    initAntutuHomeActivityMainResourceId();
		}else if (mIsAntutuDeviceInfoActivity){
			initAntutuDeviceInfoResourceId();
		}
	}
	private CharSequence filterAntutuTextView(CharSequence text) {
        if (mIsAntutuActivityTestResultDetails) {
			text = filterAntutuTestResultDetailsScoreTextView(text);
        }else if (mIsAntutuHomeActivityMain){
            text = filterAntutuHomeActivityMainScoreTextView(text);
        }
		return text;
	}
    private boolean isAntutuApk(String packageName) {
		if (ANTUTU_PACKAGENAME.equals(packageName)) {
			return true;
		}
		return false;
	}
    private boolean isAntutuActivityTestResultDetails(String packageName) {
		if ("com.antutu.benchmark.ui.test.activity.ActivityTestResultDetails".equals(packageName)) {
			return true;
		}
		return false;
	}
    private boolean isAntutuHomeActivityMain(String packageName) {
		if ("com.antutu.benchmark.ui.home.activity.ActivityMain".equals(packageName)) {
			return true;
		}
		return false;
	}
	
	private boolean isAntutuDeviceInfoActivity(String className) {
		if ("com.antutu.benchmark.ui.device.activity.ActivityDeviceInfo".equals(className)) {
			return true;
		} 
		return false;
	}
	
	private String getCamReplaceSize(int size) {
		if(size<=0) return "";
		long temp = (long) Math.sqrt(size*100*10000/12.0);
		long w = temp*4;
		long h = temp*3;
		String result = w+"x"+h;
		return result;
	...
	private CharSequence findTitleAndReplaceValue(CharSequence text,String titletext,String replaceValue){
        View mView = (View)this.getParent();
        if(mView instanceof ViewGroup){
            int vcount = ((ViewGroup)mView).getChildCount();
            for (int i=0;i<vcount;i++) {
                View title = ((ViewGroup)mView).getChildAt(i);
                if(title instanceof TextView) {
                    if (((TextView)title).getText().toString().trim().equals(titletext.trim())){
                        text = replaceValue;
                        break;
                    }
                }
	...
	private CharSequence filterAntutuDeviceInfoActivityTextView(CharSequence text) {
        int id = getId();
        if (id == mAntutuDeviceInfoDescID) {
			int cameraBackSizePersent= SystemProperties.getInt("persist.sys.bcamera_pixel",0)/100;
			int cameraFrontSizePersent= SystemProperties.getInt("persist.sys.fcamera_pixel",0)/100;

			String bCamReplaceSize = getCamReplaceSize(cameraBackSizePersent);
			String fCamReplaceSize = getCamReplaceSize(cameraFrontSizePersent);
			if (cameraBackSizePersent != 0) {
                text = findTitleAndReplaceValue(text,getContext().getResources().getText(mAntutuStrRearCam).toString(),cameraBackSizePersent+" MP("+bCamReplaceSize+")");
			}
			if (cameraFrontSizePersent != 0) {
                text = findTitleAndReplaceValue(text,getContext().getResources().getText(mAntutuStrFrontCam).toString(),cameraFrontSizePersent+" MP("+fCamReplaceSize+")");
			}
        }
    ...
    private CharSequence filterAntutuHomeActivityMainScoreTextView(CharSequence text){
        if (text == null) {
            text = "";
        }
        int id = getId();
        ...
        int totalscore = getIntFromString(value);

        String repacescoreLz = SystemProperties.get("persist.sys.lz.score", "112230");
        int  repacescore=  Integer.parseInt(repacescoreLz);

        if (id == mAntutuHomeTotalScoreId&&repacescore!=-1) {
            totalscore = getIntFromString(value);
            text = text.toString().replace(value, ""+repacescore);
        }

        return text;
    }
    private CharSequence filterAntutuTestResultDetailsScoreTextView(CharSequence text){
        if (text == null) {
            text = "";
        }
        int id = getId();
        ...
        String value = text.toString();
        int score = getIntFromString(value);
        int totalscore = getIntFromString(value);
        int startindex = value.indexOf("越");
        int startindex_en = value.indexOf("d");
        int endindex = value.indexOf("%");

        String repacescoreLz = SystemProperties.get("persist.sys.lz.score", "112230");
        int  repacescore=  Integer.parseInt(repacescoreLz);
        int repacescorepercent = SystemProperties.getInt("persist.sys.antutu.percent",98);

        if (id == mAntutuDetailTotalScoreId&&repacescore!=-1) {
            totalscore = getIntFromString(value);
            text = text.toString().replace(value, ""+repacescore);
            mMultiple = (float)(repacescore*1.0)/totalscore;
        }

        if (id == mAntutuDetailScoreId&&repacescore!=-1){
			if(value.length() ==1){
				  if(value.equals("0")){
				   text = text.toString().replace(value, "3600");
				}
			}
            if (value.length()>=3&&value.length()<=5&&score != Integer.MAX_VALUE){
                text = text.toString().replace(value, ""+(int)(score*mMultiple));
            }
        }
        if (value.contains("超越")&&value.contains("%的用户")&&repacescorepercent!=-1){
            text = text.toString().replace(value.substring(startindex + 1, endindex), ""+repacescorepercent);
        }
		if (value.contains("Defeated")&&value.contains("% of users")&&repacescorepercent!=-1){
            text = text.toString().replace(value.substring(startindex_en + 1, endindex), ""+repacescorepercent);
        }
        return text;
    }    //luozan add for antutu  跑分 end
   //luozan add for 判断sd卡是否准备并且获取sd卡大小 {{&& start
    /** 
		* 获得SD卡总大小 
	*/
	private String getSDTotalSize(Context context) { 
		if(LzEnvironment.isInternalSdMounted()) {  
			StatFs stat = new StatFs(SystemProperties.get("persist.sys.external_storage")); 
	...
	*/
	...
	@hide
	*/
	private String getRamTotalMemory() {
	...
	*/
	private String getRamAvailMemory() {
		...
		float availMemlz =(Float.parseFloat(String.valueOf(availMem)))/1024/1024/1024;
		return String.format("%.2f" + " GB", availMemlz);
	} //zs add end
	
    /**
	@hide
	*/
	private String getInternalStorageAvailableSpace(){
		File path = Environment.getDataDirectory();
		StatFs stat = new StatFs(path.getPath());
		long blockSize = stat.getBlockSize();
		long availableBlocks = stat.getAvailableBlocks();
		float ret = (Float.parseFloat(String.valueOf(availableBlocks * blockSize)))/1024/1024/1024;
